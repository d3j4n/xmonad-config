...
-- preset keybindings.
myLauncher = "$(yeganesh -x -- -fn '-misc-fixed-*-*-*-*-10-*-*-*-*-*-*-*' -nb '#000000' -nf '#FFFFFF' -sb '#7C7C7C' -sf '#CEFFAC')"
...
myManageHook = composeAll
    [ className =? "epiphany"       --> doShift "2:web"
...
myKeys conf@(XConfig {XMonad.modMask = modMask}) = M.fromList $
  ...
  -- Shrink the master area.import XMonad.Config.Gnome
  , ((modMask, xK_h),
     sendMessage Shrink)
